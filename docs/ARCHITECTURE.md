...
- Pageable endpoints default to page 0, size 20, sorted by `id ASC` unless request parameters override them; `QUERY /api/invoices` is a safe/idempotent read equivalent for invoice pagination and returns `Accept-Query: application/x-www-form-urlencoded`.
